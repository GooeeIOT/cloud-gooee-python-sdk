...
def get_building(building_id=None):
    ...
    if building_id:
        response = client.get('/buildings/{}'.format(building_id))
        ...
        if action == 'l':
            # List Buildings
            response = client.get('/buildings?limit=100')
            if response.status_code != 200:
                raise Exception('[{}]: {}'.format(response.status_code, response.text))
        ...
        if not building:
            ...
            response = client.post('/buildings', data=building)
...
def get_space(building, space_id=None):
    """Identifies and provides the Space."""
    # Fetch Space of provided ID
    if space_id:
        response = client.get('/spaces/{}'.format(space_id))
        if response.status_code != 200:
            raise Exception('[{}]: {}'.format(response.status_code, response.text))
    else:
        ...
        if action == 'l':
            # List Spaces
            response = client.get('/spaces?building={}&limit=100'.format(building['id']))
            if response.status_code != 200:
                raise Exception('[{}]: {}'.format(response.status_code, response.text))
        ...
        if not space:
            ...
            response = client.post('/spaces', data=space)
            if response.status_code != 201:
                raise Exception('[{}]: {}'.format(response.status_code, response.text))
            space = response.json
...
def get_gateway(building, space):
    ...
    if action == 'l':
        # List Gateways
        response = client.get('/devices?building={}&space={}&limit=100'
    ...
    if not gateway:
        ...
        response = client.post('/devices', data=space)
        if response.status_code != 201:
            raise Exception('[{}]: {}'.format(response.status_code, response.text))
...
def get_customer():
    ...
    if not len(response.json):
        ...
        response = client.post('/customers', data=customer)
        if response.status_code != 201:
            raise Exception('[{}]: {}'.format(response.status_code, response.text))
...
def get_partner():
    ...
    response = client.get('/partners?is_manufacturer=True')
    if response.status_code != 200:
        raise Exception('[{}]: {}'.format(response.status_code, response.text))
    if not len(response.json):
...
def get_product(device_type):
    """Identifies a random Product for the Device Type."""
    response = client.get('/products?type={}&active=True'.format(device_type))
    if response.status_code != 200:
        raise Exception('[{}]: {}'.format(response.status_code, response.text))
    ...
    if not len(response.json):
        ...
        response = client.post('/products', data=product)
        if response.status_code != 201:
            raise Exception('[{}]: {}'.format(response.status_code, response.text))
        ...
        response = client.patch('/products/{}'.format(product['id']), data={'specs': new_specs})
        if response.status_code != 200:
            raise Exception('[{}]: {}'.format(response.status_code, response.text))
        product = response.json

        response = client.put('/products/{}/activate'.format(product['id']))
...
if __name__ == '__main__':
    ...
    # Configure clients
    client = GooeeClient(api_base_url=options['api'])
    client.authenticate(options['user'], options['pass'])

    # Identify Building
    building = get_building(options['building'])

    # Identify Space
    space = get_space(building, options['space'])

    # Create Gateway
    gateway = get_gateway(building, space)

    # Create WIMs
    count = input('How many Devices will you commission? ')
    for c in range(0, int(count)):
        create_device(building, space, gateway)
